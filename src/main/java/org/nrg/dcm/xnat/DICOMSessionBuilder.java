...
public class DICOMSessionBuilder extends SessionBuilder implements Closeable {
    ...
    private SortedSet<DicomAttributeIndex> setBeanFactoriesBuildersAttrsAndTags() {
        ...
        sessionTypeAttrs.put(XnatPhotoacousticimagingsessiondataBean.class, PHOTOACOUSTICIMAGINGSessionAttributes.get());
        sessionBeanBuilders.put(XnatPhotoacousticimagingsessiondataBean.class, ImmutableMap.copyOf(imageSessionBeanBuilders));
    ...
    public XnatImagesessiondataBean call() throws IOException,NoUniqueSessionException,SQLException {
        ...
        try {
            ...
            for (final Map<DicomAttributeIndex,String> entry : seriesIds) {
                ...
                final String seriesNumber = StringUtils.defaultIfBlank(entry.get(SeriesNumber), seriesInstanceUID.replaceAll("[^\\w]", "_"));
                if (!seriesToUID.containsKey(seriesNumber)) {
                    seriesToUID.put(seriesNumber, new TreeMap<>());
                }
                if (!seriesToUID.get(seriesNumber).containsKey(seriesInstanceUID)) {
                    seriesToUID.get(seriesNumber).put(seriesInstanceUID, new Series(seriesNumber, seriesInstanceUID));
                }
                final Series series = seriesToUID.get(seriesNumber).get(seriesInstanceUID);
